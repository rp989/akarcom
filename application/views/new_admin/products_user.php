...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        table = $('#table').DataTable({
            ...
            "select": true,
            "ajax": {
                "url": "<?php echo base_url()?>MemberArea/user_product/<?=@$this->uri->segment(3)?>/<?php if(isset($isUserActive)) echo $isUserActive?>",
                "type": "POST"
            },
            "language": {
                "url": "//cdn.datatables.net/plug-ins/1.10.16/i18n/Arabic.json"
    ...
    $(document).on('click', '.accepted-gsm', function () {
        var post_id = this.id;
        var button = this;
        $(this).html('<i class="fa fa-spinner fa-pulse fa-fw"></i>');
        $.ajax({
            ...
            method: "post",
            data: {post: post_id , id:<?=@$this->uri->segment(3)?>},
            success: function (response) {
                $(button).html("تمت عملية");
                $(button).attr('disabled', 'disabled');
            }
