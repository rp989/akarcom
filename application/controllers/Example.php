...
//        // set response format: xml or json, default json
    ...
    private function sendActivateData(){
        $this->load->model('UsersAuth');
        $msg = 'We sorry for not sending the activation code when you created your account. Your account has been successfully activated';
        $users = $this->UsersAuth->getNotActivateProfileUser();
        $usersActivated = array();
        foreach ($users as $user){
            $gsm = (int) $user['u_gsm'];
            $gsm = '963'.$gsm;
            $usersActivated[] = $user['u_id'];
//            $this->UsersAuth->sendSMS($msg,$gsm);
            $this->UsersAuth->sendSMS($msg,$gsm);
            $this->UsersAuth->getActivateForNotActivateProfileUser($user['u_id']);
        }

        echo json_encode($usersActivated);
    }

    public function voice(){
        $to = '16193467257';
        $brand = 'nayotex';
        $sender_id = 'technology'; //optional 411000
    ...
    public function checkVoice(){
        $request_id = '53e5316181da4db9a718882b26321578';
        $code = '411000';
//        $ip_address = 'xxxxxxxxxxx'; //optional
